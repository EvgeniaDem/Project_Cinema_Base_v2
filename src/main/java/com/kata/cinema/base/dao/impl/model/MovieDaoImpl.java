...
public class MovieDaoImpl extends AbstractDaoImpl<Long, Movies> implements MovieDao {
    ...
    public List<SearchMovieDto> getSearchMoviesWithFilter(String filterPattern) {
        return entityManager.createQuery("select new com.kata.cinema.base.models.dto.SearchMovieDto(m.id, m.name, m.originName, c.contentUrl, m.dateRelease, (select cast(sum(s.score) as double)/count(s) as avgScore from Score s where m.id = s.movie.id))" +
